...
def text_match(textMatch: TextMatch, ocrResults: List[OcrResult]) -> Position | None:
    ...
    for ocrResult in ocrResults:
        if textMatch.text.search(ocrResult.text):
            if textMatch.position is not None and not is_position_contained(textMatch.position, ocrResult.position,):
                continue
...
def image_match(imageMatch: ImageMatch, img: np.ndarray) -> Position | None:
    ...
    if imgPosition is not None:
        if imageMatch.position is not None and not is_position_contained(imageMatch.position, imgPosition):
            return None
        return imgPosition
...
class Page(BaseModel):
    ...
    def __call__(self, img: np.ndarray, ocrResults: List[OcrResult]) -> bool:
        ...
        # 清空匹配位置
        self.matchPositions = {}
        for textMatch in self.targetTexts:  # 遍历目标文本 如果匹配到目标文本则记录位置 否则返回False
            if position := text_match(textMatch, ocrResults):
                self.matchPositions[textMatch.name] = position
            else:
                return False
        for textMatch in self.excludeTexts:  # 遍历排除文本 如果匹配到排除文本则返回False
            if text_match(textMatch, ocrResults):
                return False
        for imageMatch in self.targetImages:  # 遍历目标图片 如果匹配到目标图片则记录位置 否则返回False
            if position := image_match(imageMatch, img):
                self.matchPositions[imageMatch.name] = position
            else:
                return False
        for imageMatch in self.excludeImages:  # 遍历排除图片 如果匹配到排除图片则返回False
